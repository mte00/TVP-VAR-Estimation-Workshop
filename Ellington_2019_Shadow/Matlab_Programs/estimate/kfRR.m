function [S0,P0,P1] = kfRR(Y,X,Q,B,SI,PI,T,N,L)
% function [S0,P0,P1] = KFR(Y,X,Q,B,H,SI,PI,T,N,L);
% This file performs the forward kalman filter recursions for the random coefficients VAR. 
% R = SIGMA_OLS
% C is zero
%
...
if size(Y,1)>1
    ...
    % date 1
    P1(:,:,1) = PI; % P(1|0)
    R = B; % H is shifted one period rel to observables
    K = (P1(:,:,1)*X(:,:,1))*inv( X(:,:,1)'*P1(:,:,1)*X(:,:,1) + R); % K(1)
    P0(:,:,1) = P1(:,:,1) - K*(X(:,:,1)'*P1(:,:,1)); % P(1|1)
    S0(:,1) = SI + K*( Y(:,1) - X(:,:,1)'*SI ); % S(1|1)
    % Iterating through the rest of the sample
